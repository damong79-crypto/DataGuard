...
자세한 배경·요구사항은 [PRD](PRD_DB정합성체크앱.md), 사용법은 [사용자 매뉴얼](docs/사용자_매뉴얼.md) 참고.

## 주요 기능

- **연결/쿼리 관리**: SQL Server·PostgreSQL 연결 등록·편집·삭제 + 연결 테스트
- **수동 실행**
  - **지금 실행**: 목록에서 **선택한 쿼리 1건**만 점검
  - **전체 실행**: 사용 중인 **모든 쿼리를 순차 점검** (한 건 실패해도 나머지 계속, 완료 시 실행/이상/오류 건수 요약)
- **스케줄 자동 실행**: 쿼리별 매분/매일/평일 (트레이 상주 중)
- **이력**: 영속 저장 + 필터 조회(쿼리·상태·기간) + CSV 내보내기 + 보관기간 자동 정리
- **알림**: 이상·오류 시 이메일 발송(쿼리별 정책) + 트레이 풍선
- **상주**: 트레이 최소화, Windows 시작 시 자동 실행
